...
class ReviewService(BaseService):
    ...
    def _get_booking_vendor(self, db: Session, booking: Booking) -> uuid.UUID:
    ...
    def create(
        ...
        # 3. Resolve vendor
        vendor_id = self._get_booking_vendor(db, booking)
    ...
    def get(self, db: Session, review_id: uuid.UUID) -> Review:
    ...
    def list_vendor(
        ...
        if current_user and (current_user.is_superuser or self._is_vendor_member(db, current_user, vendor_id)):
            if status:
    ...
    def reply(
        ...
        if not self._is_vendor_member(db, current_user, review.vendor_id):
            raise HTTPException(status_code=403, detail="Not authorized")
    ...
    def delete_reply(
        ...
        if not self._is_vendor_member(db, current_user, review.vendor_id) and not current_user.is_superuser:
            raise HTTPException(status_code=403, detail="Not authorized")
...
def create_review(
    ...
    return get_review_service().create(db, body, current_user, request)


def get_review(db: Session, review_id: uuid.UUID) -> Review:
    return get_review_service().get(db, review_id)
...
def list_vendor_reviews(
    ...
    return get_review_service().list_vendor(db, vendor_id, status, current_user)
...
def list_my_reviews(
    ...
    return get_review_service().list_my(db, current_user)
...
def update_review(
    ...
    return get_review_service().update(db, review_id, body, current_user, request)
...
def moderate_review(
    ...
    return get_review_service().moderate(db, review_id, body, current_user, request)
...
def reply_to_review(
    ...
    return get_review_service().reply(db, review_id, body, current_user, request)
...
def delete_reply(
    ...
    return get_review_service().delete_reply(db, review_id, current_user, request)
