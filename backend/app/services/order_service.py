...
def _calculate_shipping(items: list[tuple[Product, int]]) -> float:
    """
...
def create_order(
    ...
    for cart_item in body.items:
        ...
        line_total = _round(float(product.price) * cart_item.quantity)
        subtotal = _round(subtotal + line_total)
    ...
    # 5. Shipping cost
    shipping_cost = _calculate_shipping(item_tuples)

    # 6. Platform fee and total
    discounted_subtotal = _round(subtotal - discount_amount)
    platform_fee = _round(discounted_subtotal * settings.platform_order_commission_rate)
    total = _round(discounted_subtotal + platform_fee + shipping_cost)
...
def get_order(
    ...
    if (
        ...
        and not _is_vendor_member(db, current_user, order.vendor_id)
    ):
...
def list_vendor_orders(
    ...
    if not _is_vendor_member(db, current_user, vendor_id):
        raise HTTPException(status_code=403, detail="Not authorized")
...
def ship_order(
    ...
    if not _is_vendor_member(db, current_user, order.vendor_id):
        raise HTTPException(status_code=403, detail="Not authorized")
...
def deliver_order(
    ...
    if not _is_vendor_member(db, current_user, order.vendor_id) and not current_user.is_superuser:
        raise HTTPException(status_code=403, detail="Not authorized")
...
def cancel_order(
    ...
    is_vendor = _is_vendor_member(db, current_user, order.vendor_id)
